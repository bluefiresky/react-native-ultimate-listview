...
export default class AnimatedRefreshableScrollView extends React.Component {
  static defaultProps = {
    horizontal: false,
    scrollEnabled: true,
    header: null,
  ...
  onScroll = (event) => {
    ...
    if (this._dragFlag) {
      ...
      setTimeout(() => this._scrollview._component.scrollTo({ x: 0, y: refreshViewHeight, animated: false }), 100)
    }
  }
  ...
  onScrollEndDrag = (event) => {
    ...
    if (!this._isRefreshing) {
      if (this.state.refreshStatus === RefreshStatus.releaseToRefresh) {
        ...
        this._scrollview._component.scrollTo({ x: 0, y: refreshViewHeight, animated: true })
      }
    } else if (y <= refreshViewHeight) {
      this._scrollview._component.scrollTo({ x: 0, y: 0, animated: true })
    }
  ...
  scrollTo = (option) => {
    this._scrollview._component.scrollTo(option)
  }

  scrollToEnd = (option) => {
    this._scrollview._component.scrollToEnd(option)
  }
  ...
  onRefreshEnd = () => {
    ...
    if (this.state.refreshStatus === RefreshStatus.refreshing) {
      ...
      setTimeout(() => {
        if (this._scrollview._component.scrollTo) {
          this._scrollview._component.scrollTo({ x: 0, y: this.props.refreshViewHeight, animated: true })
        }
        this.setState({
          refreshStatus: RefreshStatus.pullToRefresh,
          refreshTitle: this.props.refreshableTitlePull,
